  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Your earnings from consultations</CardDescription>
      </CardHeader>
      <CardContent>
                ...
                {walletData.transactions.map((transaction, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-muted rounded-md">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {transaction.type === 'deposit' ? 'Payment Received' : 'Withdrawal'}
                        </p>
                    ...
                    <p className={`font-semibold ${transaction.type === 'deposit' ? 'text-green-600' : 'text-red-600'}`}>
                      {transaction.type === 'deposit' ? '+' : '-'}{formatAmount(transaction.amount, walletData.currency)}
                    </p>
                  </div>
                ))}
              </div>
